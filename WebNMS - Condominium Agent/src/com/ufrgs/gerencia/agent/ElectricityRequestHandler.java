...
public class ElectricityRequestHandler extends SimpleRequestHandler{
	...
	private final static int[]electricityOidRep = {1,3,6,1,4,1,12619,1,6};

	/**
	 * To get OID array of this Scalar Group
	* @return OID array of this Scalar Group
	*/
	public static int[] getElectricityOidRep(){
		return electricityOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getElectricityOidRep();
	}
	...
	*/
	protected int[] getSubidList(){
		int[] subidList = {1,3,5,6,7,8};
		return subidList;
	}

	ElectricityInstrument instrument = null;

	public void setElectricityInstrument(ElectricityInstrument instrument){
		this.instrument = instrument;
		this.addInstrumentHandler(instrument);
	}

	public ElectricityRequestHandler(CondominioAgent agentRef){
		agentName = agentRef;
	}
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("ELAPCOUNT",new Integer(1));
			objectHash.put("ELBULBCOUNTER",new Integer(3));
			objectHash.put("ELCONSUMPTION",new Integer(5));
			objectHash.put("ELCONSUMPTIONLIMIT",new Integer(6));
			objectHash.put("ELINSTANTCONSUMPTION",new Integer(7));
			objectHash.put("ELAPCONSUMPTIONLIMIT",new Integer(8));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("ELAPCOUNT",new Integer(2));
			objectTypeHash.put("ELBULBCOUNTER",new Integer(2));
			objectTypeHash.put("ELCONSUMPTION",new Integer(4));
			objectTypeHash.put("ELCONSUMPTIONLIMIT",new Integer(4));
			objectTypeHash.put("ELINSTANTCONSUMPTION",new Integer(4));
			objectTypeHash.put("ELAPCONSUMPTIONLIMIT",new Integer(4));
		}
	...
	protected void processGetRequest(Vector listOfVarbinds,
		...
		for(int i=0;i<nodeSize;i++){
			try{
				...
				if(oid.length != electricityOidRep.length + 2){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}

				int[] inst = AgentUtil.getInstance(oid,electricityOidRep.length);
				if(inst[1] != 0){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
			...
			try{
				...
				switch(node.getSubId()){
					...
					case ELCONSUMPTIONLIMIT:	  
						...
						String elConsumptionLimitStr = (String )var.getVarObject();
						toSendHash.put(new Integer(ELCONSUMPTIONLIMIT),elConsumptionLimitStr);

						indexList.removeElement(new Integer(ELCONSUMPTIONLIMIT));
						indexList.addElement(new Integer(ELCONSUMPTIONLIMIT));
						break;
					...
					case ELAPCONSUMPTIONLIMIT:	  
						...
						String elApConsumptionLimitStr = (String )var.getVarObject();
						toSendHash.put(new Integer(ELAPCONSUMPTIONLIMIT),elApConsumptionLimitStr);

						indexList.removeElement(new Integer(ELAPCONSUMPTIONLIMIT));
						indexList.addElement(new Integer(ELAPCONSUMPTIONLIMIT));
						break;
	...
	public void getValuesFromInstrument(VarBindRequestEvent pe, Vector varbVector,Vector columns,Vector failedList) {
		...
		for(int j=0; j< size; j++){
			try{
				...
				switch(columnID){
					case ELAPCOUNT:	  
						toReturn = valuesHash.get(new Integer(ELAPCOUNT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case ELBULBCOUNTER:	  
						toReturn = valuesHash.get(new Integer(ELBULBCOUNTER));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case ELCONSUMPTION:	  
						toReturn = valuesHash.get(new Integer(ELCONSUMPTION));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case ELCONSUMPTIONLIMIT:	  
						toReturn = valuesHash.get(new Integer(ELCONSUMPTIONLIMIT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
					...
					case ELINSTANTCONSUMPTION:	  
						toReturn = valuesHash.get(new Integer(ELINSTANTCONSUMPTION));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
					...
					case ELAPCONSUMPTIONLIMIT:	  
						toReturn = valuesHash.get(new Integer(ELAPCONSUMPTIONLIMIT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
						SnmpString var7 = new SnmpString((String )toReturn);
			...
			varb.setObjectID(AgentUtil.getScalarSnmpOID(electricityOidRep, columnID));
		}
	}
