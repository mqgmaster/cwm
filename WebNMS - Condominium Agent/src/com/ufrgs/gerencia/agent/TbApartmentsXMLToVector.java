...
public class TbApartmentsXMLToVector implements UpdateListener{
	...
	final static String [] columnNames = {"tbApEntryIndex" , "tbApEntryNumber" , "tbApEntryOwner" , "tbApEntryRooms" , "tbApEntryPeople" , "tbApEntrySector"  };
	...
	private TbApartmentsRequestHandler handler = null;
	...
	public String getColumnValue(TbApEntry entry, String columnName){
		...
		try{
			switch(columnId){
				case TBAPENTRYINDEX:
					toRet = ""+entry.getTbApEntryIndex();
					break;
				case TBAPENTRYNUMBER:
					toRet = ""+entry.getTbApEntryNumber();
					break;
				case TBAPENTRYOWNER:
					toRet = ""+entry.getTbApEntryOwner();
					break;
				case TBAPENTRYROOMS:
					toRet = ""+entry.getTbApEntryRooms();
					break;
				case TBAPENTRYPEOPLE:
					toRet = ""+entry.getTbApEntryPeople();
					break;
				case TBAPENTRYSECTOR:
					toRet = ""+entry.getTbApEntrySector();
					break;
			}
		}catch(Exception e){
	...
	public TbApartmentsXMLToVector(String dirStr, String fileName, CondominioAgent agentRef){
		...
		if(!f.exists()){ // If the Directory does not exist, create it.
		 ...
		 if (fileName == null){
			 this.fileName = "TbApartments.xml";
		}
		else{
					...
					for(Enumeration e = rowNode.children();e.hasMoreElements();){
						...
						switch(columnId){
							case TBAPENTRYINDEX:	  
								...
								entry.tbApEntryIndex = new Integer(columnValue);
								break;
							case TBAPENTRYNUMBER:	  
								entry.tbApEntryNumber = new Integer(columnValue);
								break;
							case TBAPENTRYOWNER:	  
								entry.tbApEntryOwner = new String(columnValue);
								break;
							case TBAPENTRYROOMS:	  
								entry.tbApEntryRooms = new Integer(columnValue);
								break;
							case TBAPENTRYPEOPLE:	  
								entry.tbApEntryPeople = new Integer(columnValue);
								break;
							case TBAPENTRYSECTOR:	  
								entry.tbApEntrySector = new Integer(columnValue);
								break;
						} //end of switch
	...
	public void writeIntoFile(){
		...
		for(int i = 0;i < tableVector.size();i++){
			...
			ent = (TbApEntry )tableVector.elementAt(i);
	...
	public void setTableRequestHandler(TbApartmentsRequestHandler handler){
