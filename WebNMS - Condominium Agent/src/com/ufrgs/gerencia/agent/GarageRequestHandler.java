...
public class GarageRequestHandler extends SimpleRequestHandler{

	final static int GGCOUNT = 1;

	private final static int[]garageOidRep = {1,3,6,1,4,1,12619,1,4};

	/**
	 * To get OID array of this Scalar Group
	* @return OID array of this Scalar Group
	*/
	public static int[] getGarageOidRep(){
		return garageOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getGarageOidRep();
	}
	...
	*/
	...
	GarageInstrument instrument = null;

	public void setGarageInstrument(GarageInstrument instrument){
		this.instrument = instrument;
		this.addInstrumentHandler(instrument);
	}

	public GarageRequestHandler(CondominioAgent agentRef){
		agentName = agentRef;
	}
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("GGCOUNT",new Integer(1));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("GGCOUNT",new Integer(2));
		}
	...
	protected void processGetRequest(Vector listOfVarbinds,
		...
		for(int i=0;i<nodeSize;i++){
			try{
				...
				if(oid.length != garageOidRep.length + 2){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}

				int[] inst = AgentUtil.getInstance(oid,garageOidRep.length);
				if(inst[1] != 0){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
	...
	public void getValuesFromInstrument(VarBindRequestEvent pe, Vector varbVector,Vector columns,Vector failedList) {
		...
		for(int j=0; j< size; j++){
			try{
				...
				switch(columnID){
					case GGCOUNT:	  
						toReturn = valuesHash.get(new Integer(GGCOUNT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
			...
			varb.setObjectID(AgentUtil.getScalarSnmpOID(garageOidRep, columnID));
		}
	}
