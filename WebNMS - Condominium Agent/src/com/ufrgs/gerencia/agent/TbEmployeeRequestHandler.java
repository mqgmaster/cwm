...
public class TbEmployeeRequestHandler extends BaseTableRequestHandler{
	...
	private final static int[]tbEmployeeOidRep = {1,3,6,1,4,1,12619,1,3,2,1};

	/**
	 * To get OID array of this Table Group
	* @return OID array of this Table Group
	*/
	public static int[] getTbEmployeeOidRep(){
		return tbEmployeeOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getTbEmployeeOidRep();
	}
	...
	**/
	protected int[] getSubidList(){
		int[] subidList = {1,2,3,4,5,6,7};
		return subidList;
	}

	public TbEmployeeRequestHandler(CondominioAgent agentRef){

		tModelComplete =  new AgentTableModel();
		agentName = agentRef;

		TbEmployeeXMLToVector xmlToVector = new TbEmployeeXMLToVector(CondominioAgent.agentDir + File.separator + "xmlFiles" + File.separator +  "CONDOMINIO-MIB", "TbEmployee.xml", agentName);
		try{
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("TBEPENTRYINDEX",new Integer(1));
			objectHash.put("TBEPENTRYNAME",new Integer(2));
			objectHash.put("TBEPENTRYROLE",new Integer(3));
			objectHash.put("TBEPENTRYWAGE",new Integer(4));
			objectHash.put("TBEPENTRYWORKLOAD",new Integer(5));
			objectHash.put("TBEPENTRYWORKING",new Integer(6));
			objectHash.put("TBEPENTRYCONDOMINIUM",new Integer(7));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("TBEPENTRYINDEX",new Integer(2));
			objectTypeHash.put("TBEPENTRYNAME",new Integer(4));
			objectTypeHash.put("TBEPENTRYROLE",new Integer(4));
			objectTypeHash.put("TBEPENTRYWAGE",new Integer(2));
			objectTypeHash.put("TBEPENTRYWORKLOAD",new Integer(2));
			objectTypeHash.put("TBEPENTRYWORKING",new Integer(2));
			objectTypeHash.put("TBEPENTRYCONDOMINIUM",new Integer(2));
		}
	...
	*/
	protected Vector processReadRequest(VarBindRequestEvent pe,
					TbEpEntry entry,Vector attributesList,
		...
		for(int i=0;i<attributesList.size();i++){
			...
			if (oid.length < tbEmployeeOidRep.length + 1){
				columnID =TBEPENTRYINDEX;
			}
			else{
				columnID = oid[tbEmployeeOidRep.length];
			}
			...
			try {
				...
				switch(columnID){
					case TBEPENTRYINDEX:
						toReturn = entry.getTbEpEntryIndex();
						if(toReturn == null){
					...
					case TBEPENTRYNAME:
						toReturn = entry.getTbEpEntryName();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBEPENTRYROLE:
						toReturn = entry.getTbEpEntryRole();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
					...
					case TBEPENTRYWAGE:
						toReturn = entry.getTbEpEntryWage();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBEPENTRYWORKLOAD:
						toReturn = entry.getTbEpEntryWorkLoad();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBEPENTRYWORKING:
						toReturn = entry.getTbEpEntryWorking();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBEPENTRYCONDOMINIUM:
						toReturn = entry.getTbEpEntryCondominium();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						SnmpInt var6 = new SnmpInt(((Integer )toReturn).intValue());
			...
			SnmpOID objectId 
				= AgentUtil.getColumnarOid(tbEmployeeOidRep,columnID,entry.getInstanceOID());
	...
	*/
	public Vector processWriteRequest(String instanceString,
		...
		entry = (TbEpEntry )tModelComplete.get(instArray);
		...
		if(entry == null && !pe.isRollback()) {
			...
			try{
				entry.setTbEpEntryIndex((Integer )indexVar[0].getVarObject());
				addRow(entry);
				newRow = true;
			}catch(Exception exp){
				varb = (SnmpVarBind)list.elementAt(0);
		...
		for(int i=0;i<size;i++){
			try{
				...
				int[] oid = (int[]) varb.getObjectID().toValue();
				int columnID = oid[tbEmployeeOidRep.length];
				index =(Integer)varbindIndexHash.get(varb);
				...
				switch(columnID){
					...
					case TBEPENTRYROLE:
						...
						String tbEpEntryRoleStr = (String )var.getVarObject();
						toSendHash.put(new Integer(TBEPENTRYROLE),tbEpEntryRoleStr);

						columnsList.removeElement(new Integer(TBEPENTRYROLE));
						columnsList.addElement(new Integer(TBEPENTRYROLE));
						break;
					...
					case TBEPENTRYWAGE:
						...
						Integer tbEpEntryWageStr = (Integer )var.getVarObject();
						toSendHash.put(new Integer(TBEPENTRYWAGE),tbEpEntryWageStr);

						columnsList.removeElement(new Integer(TBEPENTRYWAGE));
						columnsList.addElement(new Integer(TBEPENTRYWAGE));
						break;
					...
					case TBEPENTRYWORKLOAD:
						...
						Integer tbEpEntryWorkLoadStr = (Integer )var.getVarObject();
						toSendHash.put(new Integer(TBEPENTRYWORKLOAD),tbEpEntryWorkLoadStr);

						columnsList.removeElement(new Integer(TBEPENTRYWORKLOAD));
						columnsList.addElement(new Integer(TBEPENTRYWORKLOAD));
						break;
	...
	*/
	protected void processGetRequest(Vector varbinds,
		...
		for (int i = 0; i < varSize; i++) {
			...
			try{
				if (oid.length < tbEmployeeOidRep.length + 2) {
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
				instArray = AgentUtil.getInstance(oid,tbEmployeeOidRep.length + 1);
	...
	protected void processGetNextRequest(Vector varbinds,
		...
		for(int j=0; j<instanceList.size() ; j++){
			...
			if(insString.equals("FIRST")){
				entry = ( TbEpEntry )tModelComplete.getFirstEntry();
			}
			else{
				instArray=utils.stringToIntArray(insString);
				entry = (TbEpEntry)tModelComplete.getNext(instArray);
			}
			while(entry != null){
				readFailedList = processReadRequest(pe,entry,attributesList,varbindIndexHash);
				if(readFailedList.size()>0){
					entry = (TbEpEntry )tModelComplete.getNext(entry.getInstanceOID());
				}
				else{
					break;
				}
	...
	*/
	private void remove(TbEpEntry entry)
	...
	*/
	private void addRow(TbEpEntry entry)
				throws AgentSnmpException{
	 ...
	 */
	public boolean createAndAddNewTbEpEntry(
					Integer valueForTbEpEntryIndex,
					String valueForTbEpEntryName,
					String valueForTbEpEntryRole,
					Integer valueForTbEpEntryWage,
					Integer valueForTbEpEntryWorkLoad,
					Integer valueForTbEpEntryWorking,
					Integer valueForTbEpEntryCondominium){

		String instString = createInstanceForTbEpEntry(
						valueForTbEpEntryIndex);
		return createAndAddTbEpEntry(instString,
 					valueForTbEpEntryIndex,
 					valueForTbEpEntryName,
 					valueForTbEpEntryRole,
 					valueForTbEpEntryWage,
 					valueForTbEpEntryWorkLoad,
 					valueForTbEpEntryWorking,
 					valueForTbEpEntryCondominium);
	}

	/**
	 * The Method to create the Instance OID for this entry.
	 */
	public String createInstanceForTbEpEntry(
					Integer valueForTbEpEntryIndex){

		int[] inst = null;
		int[] ins = null;
		int insInt = valueForTbEpEntryIndex.intValue();
		ins = new int[]{insInt};
	 ...
	 */
	public boolean createAndAddTbEpEntry(String instString, 
					Integer valueForTbEpEntryIndex, 
					String valueForTbEpEntryName, 
					String valueForTbEpEntryRole, 
					Integer valueForTbEpEntryWage, 
		...
		TbEpEntry entry = agentName.getTbEpEntryInstance();
		try{
			entry.setInstanceOID(inst);
			entry.setTbEpEntryIndex(valueForTbEpEntryIndex);
			entry.setTbEpEntryName(valueForTbEpEntryName);
			entry.setTbEpEntryRole(valueForTbEpEntryRole);
			entry.setTbEpEntryWage(valueForTbEpEntryWage);
			entry.setTbEpEntryWorkLoad(valueForTbEpEntryWorkLoad);
			entry.setTbEpEntryWorking(valueForTbEpEntryWorking);
			entry.setTbEpEntryCondominium(valueForTbEpEntryCondominium);
			addRow(entry);
		}
		catch(Exception ee){
			ee.printStackTrace();
