...
public class EmployeeRequestHandler extends SimpleRequestHandler{
	...
	final static int EPCOUNT = 1;

	private final static int[]employeeOidRep = {1,3,6,1,4,1,12619,1,3};

	/**
	 * To get OID array of this Scalar Group
	* @return OID array of this Scalar Group
	*/
	public static int[] getEmployeeOidRep(){
		return employeeOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getEmployeeOidRep();
	}
	...
	*/
	...
	EmployeeInstrument instrument = null;

	public void setEmployeeInstrument(EmployeeInstrument instrument){
		this.instrument = instrument;
		this.addInstrumentHandler(instrument);
	}

	public EmployeeRequestHandler(CondominioAgent agentRef){
		agentName = agentRef;
	}
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("EPCOUNT",new Integer(1));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("EPCOUNT",new Integer(2));
		}
	...
	protected void processGetRequest(Vector listOfVarbinds,
		...
		for(int i=0;i<nodeSize;i++){
			try{
				...
				if(oid.length != employeeOidRep.length + 2){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}

				int[] inst = AgentUtil.getInstance(oid,employeeOidRep.length);
				if(inst[1] != 0){
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
	...
	public void getValuesFromInstrument(VarBindRequestEvent pe, Vector varbVector,Vector columns,Vector failedList) {
		...
		for(int j=0; j< size; j++){
			try{
				...
				switch(columnID){
					case EPCOUNT:	  
						toReturn = valuesHash.get(new Integer(EPCOUNT));
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
			...
			varb.setObjectID(AgentUtil.getScalarSnmpOID(employeeOidRep, columnID));
		}
	}
