...
public class TbApWaterRequestHandler extends BaseTableRequestHandler{
	...
	private final static int[]tbApWaterOidRep = {1,3,6,1,4,1,12619,1,5,2,1};

	/**
	 * To get OID array of this Table Group
	* @return OID array of this Table Group
	*/
	public static int[] getTbApWaterOidRep(){
		return tbApWaterOidRep;
	}

	// This is generated to preserve the old API
	public int[] getOidRep(){
		return getTbApWaterOidRep();
	}
	...
	**/
	...
	public TbApWaterRequestHandler(CondominioAgent agentRef){

		tModelComplete =  new AgentTableModel();
		agentName = agentRef;

		TbApWaterXMLToVector xmlToVector = new TbApWaterXMLToVector(CondominioAgent.agentDir + File.separator + "xmlFiles" + File.separator +  "CONDOMINIO-MIB", "TbApWater.xml", agentName);
		try{
	...
	*/
	public Hashtable getObjectHash(){
		if(objectHash == null){
			...
			objectHash.put("TBAPWTENTRYINDEX",new Integer(1));
			objectHash.put("TBAPWTENTRYNUMBER",new Integer(2));
			objectHash.put("TBAPWTENTRYTOTALCONSUPTION",new Integer(3));
			objectHash.put("TBAPWTENTRYTOTALLIMIT",new Integer(4));
			objectHash.put("TBAPWTENTRYINSTANTCONSUPTION",new Integer(5));
			objectHash.put("TBAPWTENTRYINSTANTLIMIT",new Integer(6));
		}
		return objectHash;
	}
	...
	*/
	public Hashtable getObjectTypeHash(){
		if(objectTypeHash == null){
			...
			objectTypeHash.put("TBAPWTENTRYINDEX",new Integer(2));
			objectTypeHash.put("TBAPWTENTRYNUMBER",new Integer(2));
			objectTypeHash.put("TBAPWTENTRYTOTALCONSUPTION",new Integer(4));
			objectTypeHash.put("TBAPWTENTRYTOTALLIMIT",new Integer(4));
			objectTypeHash.put("TBAPWTENTRYINSTANTCONSUPTION",new Integer(4));
			objectTypeHash.put("TBAPWTENTRYINSTANTLIMIT",new Integer(4));
		}
	...
	*/
	protected Vector processReadRequest(VarBindRequestEvent pe,
					TbApWtEntry entry,Vector attributesList,
		...
		for(int i=0;i<attributesList.size();i++){
			...
			if (oid.length < tbApWaterOidRep.length + 1){
				columnID =TBAPWTENTRYINDEX;
			}
			else{
				columnID = oid[tbApWaterOidRep.length];
			}
			...
			try {
				...
				switch(columnID){
					case TBAPWTENTRYINDEX:
						toReturn = entry.getTbApWtEntryIndex();
						if(toReturn == null){
					...
					case TBAPWTENTRYNUMBER:
						toReturn = entry.getTbApWtEntryNumber();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBAPWTENTRYTOTALCONSUPTION:
						toReturn = entry.getTbApWtEntryTotalConsuption();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
					...
					case TBAPWTENTRYTOTALLIMIT:
						toReturn = entry.getTbApWtEntryTotalLimit();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
					...
					case TBAPWTENTRYINSTANTCONSUPTION:
						toReturn = entry.getTbApWtEntryInstantConsuption();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
					...
					case TBAPWTENTRYINSTANTLIMIT:
						toReturn = entry.getTbApWtEntryInstantLimit();
						if(toReturn == null){
							AgentUtil.throwNoSuchInstance(pe.getVersion());
						}
						// Generating for other TC
			...
			SnmpOID objectId 
				= AgentUtil.getColumnarOid(tbApWaterOidRep,columnID,entry.getInstanceOID());
	...
	*/
	public Vector processWriteRequest(String instanceString,
		...
		entry = (TbApWtEntry )tModelComplete.get(instArray);
		...
		if(entry == null && !pe.isRollback()) {
			...
			try{
				entry.setTbApWtEntryIndex((Integer )indexVar[0].getVarObject());
				addRow(entry);
				newRow = true;
			}catch(Exception exp){
				varb = (SnmpVarBind)list.elementAt(0);
		...
		for(int i=0;i<size;i++){
			try{
				...
				int[] oid = (int[]) varb.getObjectID().toValue();
				int columnID = oid[tbApWaterOidRep.length];
				index =(Integer)varbindIndexHash.get(varb);
				...
				switch(columnID){
					...
					case TBAPWTENTRYTOTALLIMIT:
						...
						String tbApWtEntryTotalLimitStr = (String )var.getVarObject();
						toSendHash.put(new Integer(TBAPWTENTRYTOTALLIMIT),tbApWtEntryTotalLimitStr);

						columnsList.removeElement(new Integer(TBAPWTENTRYTOTALLIMIT));
						columnsList.addElement(new Integer(TBAPWTENTRYTOTALLIMIT));
						break;
					...
					case TBAPWTENTRYINSTANTLIMIT:
						...
						String tbApWtEntryInstantLimitStr = (String )var.getVarObject();
						toSendHash.put(new Integer(TBAPWTENTRYINSTANTLIMIT),tbApWtEntryInstantLimitStr);

						columnsList.removeElement(new Integer(TBAPWTENTRYINSTANTLIMIT));
						columnsList.addElement(new Integer(TBAPWTENTRYINSTANTLIMIT));
						break;
	...
	*/
	protected void processGetRequest(Vector varbinds,
		...
		for (int i = 0; i < varSize; i++) {
			...
			try{
				if (oid.length < tbApWaterOidRep.length + 2) {
					AgentUtil.throwNoSuchInstance(pe.getVersion());
				}
				instArray = AgentUtil.getInstance(oid,tbApWaterOidRep.length + 1);
	...
	protected void processGetNextRequest(Vector varbinds,
		...
		for(int j=0; j<instanceList.size() ; j++){
			...
			if(insString.equals("FIRST")){
				entry = ( TbApWtEntry )tModelComplete.getFirstEntry();
			}
			else{
				instArray=utils.stringToIntArray(insString);
				entry = (TbApWtEntry)tModelComplete.getNext(instArray);
			}
			while(entry != null){
				readFailedList = processReadRequest(pe,entry,attributesList,varbindIndexHash);
				if(readFailedList.size()>0){
					entry = (TbApWtEntry )tModelComplete.getNext(entry.getInstanceOID());
				}
				else{
					break;
				}
	...
	*/
	private void remove(TbApWtEntry entry)
	...
	*/
	private void addRow(TbApWtEntry entry)
				throws AgentSnmpException{
	 ...
	 */
	public boolean createAndAddNewTbApWtEntry(
					Integer valueForTbApWtEntryIndex,
					Integer valueForTbApWtEntryNumber,
					String valueForTbApWtEntryTotalConsuption,
					String valueForTbApWtEntryTotalLimit,
					String valueForTbApWtEntryInstantConsuption,
					String valueForTbApWtEntryInstantLimit){

		String instString = createInstanceForTbApWtEntry(
						valueForTbApWtEntryIndex);
		return createAndAddTbApWtEntry(instString,
 					valueForTbApWtEntryIndex,
 					valueForTbApWtEntryNumber,
 					valueForTbApWtEntryTotalConsuption,
 					valueForTbApWtEntryTotalLimit,
 					valueForTbApWtEntryInstantConsuption,
 					valueForTbApWtEntryInstantLimit);
	}

	/**
	 * The Method to create the Instance OID for this entry.
	 */
	public String createInstanceForTbApWtEntry(
					Integer valueForTbApWtEntryIndex){

		int[] inst = null;
		int[] ins = null;
		int insInt = valueForTbApWtEntryIndex.intValue();
		ins = new int[]{insInt};
	 ...
	 */
	public boolean createAndAddTbApWtEntry(String instString, 
					Integer valueForTbApWtEntryIndex, 
					Integer valueForTbApWtEntryNumber, 
					String valueForTbApWtEntryTotalConsuption, 
					String valueForTbApWtEntryTotalLimit, 
		...
		TbApWtEntry entry = agentName.getTbApWtEntryInstance();
		try{
			entry.setInstanceOID(inst);
			entry.setTbApWtEntryIndex(valueForTbApWtEntryIndex);
			entry.setTbApWtEntryNumber(valueForTbApWtEntryNumber);
			entry.setTbApWtEntryTotalConsuption(valueForTbApWtEntryTotalConsuption);
			entry.setTbApWtEntryTotalLimit(valueForTbApWtEntryTotalLimit);
			entry.setTbApWtEntryInstantConsuption(valueForTbApWtEntryInstantConsuption);
			entry.setTbApWtEntryInstantLimit(valueForTbApWtEntryInstantLimit);
			addRow(entry);
		}
		catch(Exception ee){
			ee.printStackTrace();
